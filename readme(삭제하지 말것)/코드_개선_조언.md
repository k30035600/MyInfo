...
### 2.1 `ensure_all_bank_files` vs `ensure_bank_before_and_category`
- **현재**: 두 함수가 “bank_before 확인 → category_table.json 생성/복구/마이그레이션” 블록을 거의 동일하게 반복함. 차이는 마지막에 bank_after 생성 여부뿐.
- **조언**:  
...
### 6.2 `ensure_*` 내부의 `import shutil`
- **현재**: `except` 블록 안에서 `import shutil`을 수행.
- **조언**: 파일 상단이나 “백업/이동이 필요한 모듈” 상단에 `import shutil`을 한 번만 두고, 예외 블록에서는 사용만 하면 코드가 단순해짐.
